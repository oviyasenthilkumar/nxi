...
const handleOpenModal = (parentPath) => {
  const parentName = parentPath.split(" > ").pop(); // Extract only the last segment
  ...
  //   return false;
  // };
 const addChild = (items, parentPath, newChild, currentPath = []) => {
   return items.map((item) => {
     const newPath = [...currentPath, item.name]; // Track the full path

     if (newPath.join(" > ") === parentPath) {
       return {
         ...item,
         children: [...(item.children || []), newChild], // Append the new child
       };
     }
     ...
     if (item.children) {
       return {
         ...item,
         children: addChild(item.children, parentPath, newChild, newPath),
       };
     }
...
const handleSubmit = (e) => {
  ...
  if (formData.parent === "") {
    // Add as a new L1 parent
    setData((prevData) => [...prevData, newChild]);
  } else {
    // Add to the correct parent
    setData((prevData) => addChild(prevData, formData.parent, newChild));
  }
